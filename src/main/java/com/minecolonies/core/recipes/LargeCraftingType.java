...
public class LargeCraftingType extends RecipeCraftingType {
    public LargeCraftingType() {
        super(ModCraftingTypes.LARGE_CRAFTING_ID,
                RecipeType.CRAFTING, predicate());
    }

    private static Predicate<Recipe> predicate() {
        return r -> r.canCraftInDimensions(3, 3)
    ...
    public MenuProvider getMenuProvider(IBuilding building, AbstractCraftingBuildingModule module) {
        return new MenuProvider()
        {
            ...
            {
                return new ContainerCrafting(id, inv, true, building.getID(), module.getProducer().getRuntimeID());
            }
        };
    }
    ...
    public Consumer<FriendlyByteBuf> populateMenuBuffer(IBuilding building, AbstractCraftingBuildingModule module) {
        return buffer -> new FriendlyByteBuf(buffer.writeBoolean(true)).writeBlockPos(building.getID())
